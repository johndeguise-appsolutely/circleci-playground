...
then
  ...
  do
    ...
    packageVersionJSON=$(sfdx force:data:soql:query -q "SELECT Id,Package2Id,SubscriberPackageVersionId,Description,IsDeprecated,IsPasswordProtected,IsReleased FROM Package2Version WHERE Package2Id='$package' AND Branch='$branch' ORDER BY CreatedDate DESC LIMIT 1" -t --json -u "$devhubusername")
